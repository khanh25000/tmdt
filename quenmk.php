...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($new_password !== $confirm_password) {
        ...
        // Kiểm tra xem số điện thoại và vai trò có khớp không
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $stmt = $pdo->prepare("UPDATE users SET password = ? WHERE phone = ? AND role = ?");
            
            if ($stmt->execute([$hashed_password, $phone, $role])) {
                $success = "Đặt lại mật khẩu thành công! Bạn sẽ được chuyển hướng đến trang đăng nhập.";
                header("refresh:3;url=login.php");
            } else {
                $error = "Đã xảy ra lỗi khi đặt lại mật khẩu. Vui lòng thử lại!";
            }
        } else {
            $error = "Số điện thoại hoặc vai trò không đúng!";
        }
    }
}
?>
